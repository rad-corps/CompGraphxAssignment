...
Gizmos::Gizmos(unsigned int a_maxLines, unsigned int a_maxTris,
	...
	//	uniform mat4 ProjectionView; \
	//	uniform float time_; \
	//	uniform float heightScale_; \
	//	void main() \
	//	{ vColour = Colour; vec4 P = Position; P.y += sin(time_ + Position.x) * heightScale_; gl_Position = ProjectionView * P; }";
    ...
    // create Vertex Buffers, for each type of gizmo. Here we are reserving memory for the maximum number. 
	//It might be better to allocate this memory dynamically as required. But much simpler this way. 
	
	//generate 1 buffer and set the handle to m_lineVBO
	glGenBuffers( 1, &m_lineVBO ); 
	
	//a target of GL_ARRAY_BUFFER specifies that we want to set Vertex Attributes on the m_lineVBO vertex buffer object
	...
	//enable the two arguments we pass to the shaders (position=0, colour=1)
...
void Gizmos::draw(const glm::mat4& a_projection, const glm::mat4& a_view, float time_) {
	draw(a_projection * a_view, time_);
}

void Gizmos::draw(const glm::mat4& a_projectionView, float time_) {
	if ( sm_singleton != nullptr && (sm_singleton->m_lineCount > 0 || sm_singleton->m_triCount > 0 || sm_singleton->m_transparentTriCount > 0))
	{
		...
		//adding my own uniforms
		float heightScale_ = 1.0f;

		unsigned int heightScaleUniform = glGetUniformLocation(sm_singleton->m_shader, "heightScale_");
		unsigned int timeUniform = glGetUniformLocation(sm_singleton->m_shader, "time_");
		glUniform1f(timeUniform, clock() * 0.01);
		glUniform1f(heightScaleUniform, heightScale_);
